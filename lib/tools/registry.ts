...
const PLACEHOLDER_TOOLS: PlaceholderSpec[] = [
  ...
  { slug: "ai-meeting-notes", name: "AI Meeting Notes", category: "ai", description: "Extract action items and summaries from meeting PDFs.", extensions: ["pdf"], input: "pdf" },
  { slug: "ai-legal-summary", name: "AI Legal Summary", category: "ai", description: "Summarize legal documents in plain language.", extensions: ["pdf"], input: "pdf" },
  { slug: "ai-medical-summary", name: "AI Medical Summary", category: "ai", description: "Extract key info from medical reports.", extensions: ["pdf"], input: "pdf" },
  { slug: "ai-research-assistant", name: "AI Research Assistant", category: "ai", description: "Analyze academic papers and extract insights.", extensions: ["pdf"], input: "pdf" },
...
function buildPlaceholder(spec: PlaceholderSpec): ToolDefinition {
  return {
    ...
    keywords: [
      ...
      `${spec.name.toLowerCase()} free`,
      `${spec.name.toLowerCase()} online`,
      `${spec.name.toLowerCase()} no signup`,
      `${spec.name.toLowerCase()} no registration`,
      `${spec.name.toLowerCase()} browser`,
      `${spec.name.toLowerCase()} tool`,
      `${spec.name.toLowerCase()} converter`,
      `${spec.name.toLowerCase()} 2026`,
      ...(spec.extensions ?? []).map((e) => `${e} to pdf`),
      ...(spec.extensions ?? []).map((e) => `convert ${e}`),
    ],
    faq: [
      {
        q: `How do I use the ${spec.name} tool?`,
        a: `Select your file, choose any options, and click convert. Your files are processed securely and never shared with third parties. Everything runs in your browser.`,
      },
      {
      ...
      },
      {
        ...
        a: `Files are processed in-memory and automatically deleted after one hour. We never sell or share your documents. Your privacy is our priority.`,
      },
      {
        q: `Do I need to create an account?`,
        a: `No. You can use ${spec.name} completely free without creating an account. No signup, no registration required.`,
      },
    ],
    related: [],
  };
